...
def main():
    ...
    val_size = int(len(hf_ds) * VALIDATION_SPLIT)
    val_hf = hf_ds.select(range(val_size))
    train_hf = hf_ds.select(range(val_size, len(hf_ds)))
    
    train_dataset = HFOrpheusDataset(train_hf, model.tokenizer)
    val_dataset = HFOrpheusDataset(val_hf, model.tokenizer)
...
def collate_fn(samples):
    ...
    return {
        ...
        'nationality': [s.get('nationality', 'unknown') for s in samples],
        'speaker_id': [s.get('speaker_id', 'unknown') for s in samples],
    }
